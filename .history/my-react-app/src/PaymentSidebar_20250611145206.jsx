...
const PaymentSidebar = ({ totalAmount, orderId, onClose, cartItems }) => {
  ...
  return (
    ...
    >
      {/* Form Container */}
      <Box flex="1" maxW="400px" fontSize="lg" lineHeight="tall">
        ...
        <RadioGroup onChange={setMethod} value={method} mb={8} fontSize="lg">
        ...
        <Button mt={2} variant="ghost" width="full" onClick={onClose} fontSize="lg">
          Cancel
        </Button>
      </Box>
      ...
      >
        ...
        <VStack spacing={3} align="stretch" mb={6}>
          {cartItems.map((item) => (
            <Flex key={item.id} justify="space-between" fontSize="md" fontWeight="medium">
              <Text>{item.name}</Text>
              <Text>
        ...
        <Flex justify="space-between" fontWeight="bold" fontSize="xl">
          <Text>Total</Text>
          <Text color="red.500">${totalAmount.toFixed(2)}</Text>
        </Flex>
      </Box>
    </Flex>
  );
