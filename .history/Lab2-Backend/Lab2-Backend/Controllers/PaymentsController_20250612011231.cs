...
namespace Lab2_Backend.Controllers
{
    ...
    public class PaymentsController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<ActionResult<Payments>> PostPayment([FromBody] PaymentCreateDto paymentDto)
        {
            if (paymentDto == null)
                return BadRequest("Payment data is missing.");

            // Kontrollojmë nëse porosia ekziston
            var order = await _context.Orders.FindAsync(paymentDto.OrderID);
            if (order == null)
                return BadRequest("Order not found.");

            // Verifikim shtesë vetëm për kartelë
            if (paymentDto.PaymentMethod == "Card" && paymentDto.CardData == null)
                return BadRequest("Card information is required for Card payments.");
