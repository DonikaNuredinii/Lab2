...
namespace Lab2_Backend.Controllers
{
    ...
    public class PaymentsController : ControllerBase
    {
        ...
        {
            // Kontroll nëse Order ekziston
            var existingOrder = await _context.Orders.FindAsync(paymentDto.OrderID);
            if (existingOrder == null)
            {
                return BadRequest("Invalid OrderID – order not found.");
            }
            ...
            // Log për debug ose demo (opsionale)
            Console.WriteLine($"[INFO] FirstName: {paymentDto.FirstName}");
            Console.WriteLine($"[INFO] Card Holder: {paymentDto.CardData?.Name}");
        ...
        public async Task<IActionResult> CreateCheckoutSession([FromBody] StripeCheckoutDto dto)
        {
            var options = new SessionCreateOptions
            {
                ...
                LineItems = dto.CartItems.Select(item => new SessionLineItemOptions
                {
